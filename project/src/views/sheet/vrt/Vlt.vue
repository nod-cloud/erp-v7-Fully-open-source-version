	...
	export default {
					...
					status:[
						{
							fun:()=>{
								...
								for (let row of this.tableData){
								    if(row.mold=='vend'){
										vend=this.$calc.chain(vend).add(row.info.tpt).done();
									}else if(row.mold=='vre'){
										vre=this.$calc.chain(vre).add(row.info.tpt).done();
									}else if(row.mold=='barter'){
										barter=this.$calc.chain(barter).add(row.info.total).done();
									}
								}
								return '零售总金额:'+vend+' | '+'零售退货总金额:'+vre+' | '+'积分兑换总金额:'+barter;
							}
						},
					]
				}
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("vrt/vlt", parm).then(result => {
			...
			exports(){
				...
				setTimeout(() => {
					window.open(this.$base.web+'vrt/vltExports?'+parm)
				}, 1000);
			},
			//页面刷新
			reload() {
