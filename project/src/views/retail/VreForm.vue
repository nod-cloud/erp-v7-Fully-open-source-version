<template>
	<div class="vreForm area">
		<div class="layout">
			...
			<el-button-group>
				<template v-if="tableSelection.length!=0">
					...
					<template v-if="$lib.getUserRoot(store.root,'vre','examine')">
					...
					<template v-if="$lib.getUserRoot(store.root,'vre','del')">
						<el-button @click="del(tableSelection)"  type="info">删除</el-button>
					</template>
				</template>
				<template v-if="$lib.getUserRoot(store.root,'vre','batch')">
		...
		<el-dialog class="billDialog" :visible.sync="bill.dialog" title="单据详情" :fullscreen="true" @close="billDestroy">
			<transition name="el-fade-in">
				<template v-if="bill.dialog">
					<Vre :source="bill.source" @destroy="bill.dialog=false"></Vre>
				</template>
			</transition>
		</el-dialog>
		<el-dialog v-if="bills.type=='vend'" class="billDialog" :visible.sync="bills.dialog" title="零售单详情" :fullscreen="true" >
			<transition name="el-fade-in">
				<template v-if="bills.dialog">
					<Vend :source="bills.source" @destroy="bills.dialog=false"></Vend>
				</template>
			</transition>
		</el-dialog>
		...
		<Viewer v-if="report.dialog" mould="vre" :source="report.source" @destroy="viewerDestroy"></Viewer>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("vre/record", parm).then(result => {
					if (result.state == "success") {
			...
			downTemplate(){
				...
				setTimeout(() => {
					window.open(this.store.base.cdnSite+"/erp/batch/零售退货单导入模板.xlsx")
				}, 1000);
			},
			//上传模板回调
			...
			exports(scene){
				if(this.tableSelection.length==0){
					...
					setTimeout(() => {
						window.open(this.$base.web+'vre/exports?'+parm)
					}, 1000);
				}
			},
			//关联订单页面切换
