<template>
	<div class="vendForm area">
		<div class="layout">
			<el-popover ref="searchPopover" popper-class="searchPopover" placement="bottom-start">
				<el-form class="searchFrom" ref="searchFrom"  inline>
					...
					<el-form-item>
						<el-select v-model="searchFrom.ptm" placeholder="请选择付款方式" clearable>
					...
					<el-form-item>
						<el-input placeholder="请输入结算单号" v-model="searchFrom.ptn" clearable></el-input>
					</el-form-item>
					<el-form-item>
						<el-input placeholder="请输入发票信息" v-model="searchFrom.invoice" clearable></el-input>
					</el-form-item>
					<el-form-item>
			...
			<el-button-group>
				<template v-if="tableSelection.length!=0">
					...
					<template v-if="$lib.getUserRoot(store.root,'vend','del')">
						<el-button @click="del(tableSelection)"  type="info">删除</el-button>
					</template>
				</template>
				<template v-if="$lib.getUserRoot(store.root,'vend','batch')">
		...
		<el-table :data="tableData" height="calc(100% - 90px)" @selection-change="selectionChange" border v-madeTable>
			...
			<el-table-column prop="integral" label="单据积分" align="center" width="120px"></el-table-column>
			<el-table-column prop="cost" label="单据费用" align="center" width="120px"></el-table-column>
			<el-table-column prop="extension.ptm" label="结算方式" align="center" width="120px"></el-table-column>
		...
		<el-dialog class="billDialog" :visible.sync="bill.dialog" title="单据详情" :fullscreen="true" @close="billDestroy">
			<transition name="el-fade-in">
				<template v-if="bill.dialog">
					<Vend :source="bill.source" @destroy="bill.dialog=false"></Vend>
				</template>
			</transition>
		</el-dialog>
		<el-dialog v-if="bills.type=='vre'" class="billDialog" :visible.sync="bills.dialog" title="零售退货单详情" :fullscreen="true" >
			<transition name="el-fade-in">
				<template v-if="bills.dialog">
					<Vre :source="bills.source" @destroy="bills.dialog=false"></Vre>
				</template>
			</transition>
		</el-dialog>
...
<script>
	import Vend from "@/views/retail/Vend";
	import Vre from "@/views/retail/Vre";
	...
	export default {
			...
			return {
				...
				page: {
					...
					status:[
						...
						{text:'总单据积分',type:'sum',key:'integral'},
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("vend/record", parm).then(result => {
					if (result.state == "success") {
			...
			downTemplate(){
				...
				setTimeout(() => {
					window.open(this.store.base.cdnSite+"/erp/batch/零售单导入模板.xlsx")
				}, 1000);
			},
			//上传模板回调
			...
			exports(scene){
				if(this.tableSelection.length==0){
					...
					setTimeout(() => {
						window.open(this.$base.web+'vend/exports?'+parm)
					}, 1000);
				}
			},
			//关联订单页面切换
