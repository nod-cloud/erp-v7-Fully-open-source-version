<template>
	<div class="barterForm area">
		<div class="layout">
			...
			<el-button-group>
				<template v-if="tableSelection.length!=0">
					...
					<template v-if="$lib.getUserRoot(store.root,'barter','del')">
						<el-button @click="del(tableSelection)"  type="info">删除</el-button>
					</template>
				</template>
				<template v-if="$lib.getUserRoot(store.root,'barter','batch')">
		...
		<el-table :data="tableData" height="calc(100% - 90px)" @selection-change="selectionChange" border v-madeTable>
			...
			<el-table-column prop="actual" label="实收积分" align="center" width="120px"></el-table-column>
		...
		<el-dialog class="billDialog" :visible.sync="bill.dialog" title="单据详情" :fullscreen="true" @close="billDestroy">
			<transition name="el-fade-in">
				<template v-if="bill.dialog">
					<Barter :source="bill.source" @destroy="bill.dialog=false"></Barter>
				</template>
			</transition>
		</el-dialog>
...
<script>
	import Barter from "@/views/retail/Barter";
	...
	export default {
			...
			return {
				...
				page: {
					...
					status:[
						...
						{text:'总实收积分',type:'sum',key:'actual'},
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("barter/record", parm).then(result => {
			...
			downTemplate(){
				...
				setTimeout(() => {
					window.open(this.store.base.cdnSite+"/erp/batch/积分兑换单导入模板.xlsx")
				}, 1000);
			},
			//上传模板回调
			...
			exports(scene){
				if(this.tableSelection.length==0){
					...
					setTimeout(() => {
						window.open(this.$base.web+'barter/exports?'+parm)
					}, 1000);
				}
			},
			//表格选中数据改变
