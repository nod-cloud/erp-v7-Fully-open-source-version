<template>
	<div class="vre bill">
		<el-form :model="form" :rules="rules" ref="form" :inline="true" :hide-required-asterisk="true" style="height: 100%;">
			...
			<LineFeed tag=".el-form-item" :rule="3">
				...
				<el-form-item label="单据附件">
					<NodUpload v-model="form.file" :action="$base.web+'vre/upload'"></NodUpload>
				</el-form-item>
				<FieldForm style="display: initial;" v-model="form.more" rule="vre" prefix="more."></FieldForm>
		...
		<div class="layerBtn">
			<el-button-group>
				<template v-if="form.id==0 || (form.examine==0 && $lib.getUserRoot(store.root,'vre','edit'))">
...
<script>
	...
	export default {
					...
					column: [
						{
							...
							headerComponentParams:{dispose:{key:'vre'}},
						...
						},
						{
							...
							editable: true,
							cellEditor: 'price',
							cellEditorParams:{dispose:{model:'vre',source:()=>{return this.form.customer}}},
							hide: false,
							dispose:true
						},
						{
							...
							editable: true,
							cellEditor: 'serial',
							cellEditorParams:{dispose:{record:{show:true,parm:{state:1}}}},
							hide: false,
							dispose:true
						},
						{
		...
		created() {
			this.init();
			this.$bus.on('buildVre', this.buildVre);
		},
		methods: {
			//初始化
			init(){
				if(this.source==null){
					...
					this.form.number='LSTHD'+this.$moment().format('YYMMDDHHmmssS');
				}else{
					this.$axios.post("vre/get", {parm:this.source}).then(result => {
		...
		beforeDestroy() {
			this.$bus.off('buildVre', this.buildVre);
		}
	};
</script>