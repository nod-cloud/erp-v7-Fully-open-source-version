<template>
	<div class="sorForm area">
		<div class="layout">
			<el-popover ref="searchPopover" popper-class="searchPopover" placement="bottom-start">
				<el-form class="searchFrom" ref="searchFrom"  inline>
					...
					<el-form-item>
						<el-select v-model="searchFrom.state" placeholder="请选择出库状态" clearable>
		...
		<el-table :data="tableData" height="calc(100% - 90px)" @selection-change="selectionChange" border v-madeTable>
			...
			<el-table-column prop="extension.state" label="出库状态" align="center" width="100px"></el-table-column>
		...
		<el-dialog v-if="bills.type=='sell'" class="billDialog" :visible.sync="bills.dialog" title="销售单详情" :fullscreen="true" >
			<transition name="el-fade-in">
				<template v-if="bills.dialog">
	...
	export default {
		...
		methods: {
			...
			downTemplate(){
				...
				setTimeout(() => {
					window.open(this.store.base.cdnSite+"/erp/batch/销售订单导入模板.xlsx")
				}, 1000);
			},
			//上传模板回调
