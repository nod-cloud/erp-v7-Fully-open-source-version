<template>
	<div class="sell bill">
		<el-form :model="form" :rules="rules" ref="form" :inline="true" :hide-required-asterisk="true" style="height: 100%;">
			...
			<LineFeed tag=".el-form-item" :rule="3">
				...
				<el-form-item label="单据附件">
					<NodUpload v-model="form.file" :action="$base.web+'sell/upload'"></NodUpload>
				</el-form-item>
				<FieldForm style="display: initial;" v-model="form.more" rule="sell" prefix="more."></FieldForm>
		...
		<div class="layerBtn">
			<el-button-group>
				<template v-if="form.id==0 || (form.examine==0 && $lib.getUserRoot(store.root,'sell','edit'))">
					<el-button type="info"  @click="save">保存</el-button>
				</template>
				<template v-if="form.id!=0">
					<template v-if="$lib.getUserRoot(store.root,'sre','add') && form.examine==1">
		...
		<Viewer v-if="report.dialog" mould="sell" :source="report.source" @destroy="viewerDestroy"></Viewer>
	</div>
</template>
<script>
			...
			return {
				...
				rules: {
					...
					money: [
						...
						{
							validator: (rule,value,callback)=>{
								this.$lib.synValidate('money',value)?callback():callback(new Error('实收金额不正确'));
							},
							trigger: "blur"
						}
					]
				...
				grid: {
					...
					column: [
						...
						},
						{
							...
							editable: true,
							cellEditor: 'price',
							cellEditorParams:{dispose:{model:'sell',source:()=>{return this.form.customer}}},
							hide: false,
							dispose:true
						},
						{
