<template>
	<div class="ice bill">
		<el-form :model="form" :rules="rules" ref="form" :inline="true" :hide-required-asterisk="true" style="height: 100%;">
			...
			<LineFeed tag=".el-form-item" :rule="3">
				...
				<el-form-item label="单据附件">
					<NodUpload v-model="form.file" :action="$base.web+'ice/upload'"></NodUpload>
				</el-form-item>
				<FieldForm style="display: initial;" v-model="form.more" rule="ice" prefix="more."></FieldForm>
		...
		<div class="layerBtn">
			<el-button-group>
				<template v-if="form.id==0 || (form.examine==0 && $lib.getUserRoot(store.root,'ice','edit'))">
		...
		<Viewer v-if="report.dialog" mould="ice" :source="report.source" @destroy="viewerDestroy"></Viewer>
	</div>
</template>
<script>
		...
		data() {
			return {
				...
				grid: {
					...
					context: {
						row:{uniqid:this.$lib.getUniqid(),ietId:null,iet:'点击选择',money:'',data:''},
						runHandleGrid:()=>{
					...
					column: [
						...
						},
						{
							...
							editable: true,
							cellEditor: 'rae',
							cellEditorParams:{dispose:{key:'ietId',text:'iet',type:'in'}},
							hide: false,
							dispose:true
						},
						{
			...
			handleGrid(){
				...
				for (var i = 0; i < data.length; i++) {
					if (data[i].ietId==null) {
						effect==true&&(effect="数据表格第" + (i + 1) + "行收入类别不可为空!");
						continue;
