<template>
	<div class="bre bill">
		<el-form :model="form" :rules="rules" ref="form" :inline="true" :hide-required-asterisk="true" style="height: 100%;">
			...
			<LineFeed tag=".el-form-item" :rule="3">
				...
				<el-form-item label="单据附件">
					<NodUpload v-model="form.file" :action="$base.web+'bre/upload'"></NodUpload>
				</el-form-item>
				<FieldForm style="display: initial;" v-model="form.more" rule="bre" prefix="more."></FieldForm>
		...
		<div class="layerBtn">
			<el-button-group>
				<template v-if="form.id==0 || (form.examine==0 && $lib.getUserRoot(store.root,'bre','edit'))">
		...
		<Viewer v-if="report.dialog" mould="bre" :source="report.source" @destroy="viewerDestroy"></Viewer>
	</div>
</template>
<script>
				...
				grid: {
					...
					column: [
						...
						},
						{
							...
							editable: true,
							cellEditor: 'price',
							cellEditorParams:{dispose:{model:'bre',source:()=>{return this.form.supplier}}},
							hide: false,
							dispose:true
						},
						{
