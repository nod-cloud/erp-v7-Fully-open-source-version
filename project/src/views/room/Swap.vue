<template>
	<div class="swap bill">
		<el-form :model="form" :rules="rules" ref="form" :inline="true" :hide-required-asterisk="true" style="height: 100%;">
			...
			<LineFeed tag=".el-form-item" :rule="3">
				...
				<el-form-item label="单据附件">
					<NodUpload v-model="form.file" :action="$base.web+'swap/upload'"></NodUpload>
				</el-form-item>
				<FieldForm style="display: initial;" v-model="form.more" rule="swap" prefix="more."></FieldForm>
		...
		<div class="layerBtn">
			<el-button-group>
				<template v-if="form.id==0 || (form.examine==0 && $lib.getUserRoot(store.root,'swap','edit'))">
		...
		<Viewer v-if="report.dialog" mould="swap" :source="report.source" @destroy="viewerDestroy"></Viewer>
	</div>
</template>
<script>
				...
				grid: {
					...
					column: [
						...
						},
						{
							...
							headerComponentParams:{dispose:{title:'调出仓库',key:'warehouseId',text:'warehouse'}},
							cellRenderer: 'depot',
						...
						},
						{
							...
							field: 'storehouse',
							headerComponent: 'storeroom',
							headerComponentParams:{dispose:{title:'调入仓库',key:'storehouseId',text:'storehouse'}},
							cellRenderer: 'depot',
							cellRendererParams:{dispose:{key:'storehouseId',text:'storehouse'}},
							editable: true,
							cellEditor: 'storehouse',
							cellEditorParams:{dispose:{key:'storehouseId',text:'storehouse'}},
							hide: false,
							dispose:true
						},
						{
			...
			pushGrid(items, scene = true) {
				...
				for (let item of items) {
					...
					let row = {
						...
						storehouse: scene?(goods.type==0?'点击选择':''):this.$lib.objectSeek(item,'storehouseData|name'),
				...
				for (var i = 0; i < data.length; i++) {
					if (data[i].key == null) {
						...
						if (data[i].unitData.length>0 && data[i].unit=='点击选择') {
							...
							effect==true&&(effect="商品信息第" + (i + 1) + "行调出仓库不可为空!");
							continue;
						} else if (data[i].goodsType==0 && data[i].storehouseId==null) {
							effect==true&&(effect="商品信息第" + (i + 1) + "行调入仓库不可为空!");
							continue;
						} else if (data[i].goodsType==0 && (data[i].warehouseId == data[i].storehouseId)) {
							effect==true&&(effect="商品信息第" + (i + 1) + "行调出调入仓库不可相等!");
							continue;
