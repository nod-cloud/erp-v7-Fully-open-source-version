...
use app\model\{Barter as Barters,BarterInfo,Cost,Goods};
use think\facade\{Db,Filesystem};
use think\exception\ValidateException;
class Barter extends Acl{
    //列表
    public function record(){
        ...
        if(existFull($input,['page','limit'])){
            ...
            if(existFull($input,['goods'])){
                ...
                $sql[]=['id','in',array_column(Db::name('barter_info')->where([['goods','in',$goods]])->select()->toArray(),'pid')];
            }
            $sql=frameScope($sql);//组织数据
            $sql=sqlAuth('barter',$sql);//数据鉴权
            $count = Barters::where($sql)->count();//获取总条数
            $info = Barters::with(['frameData','customerData','peopleData','userData','costData','recordData'])->where($sql)->append(['extension'])->page($input['page'],$input['limit'])->order(['id'=>'desc'])->select()->toArray();//查询分页数据
            $result=[
                'state'=>'success',
                'count'=>$count,
                'info'=>$info
    ...
    public function save(){
        ...
        if(existFull($input,['class','info']) && isset($input['class']['id']) && isset($input['cost'])){
            ...
            try {
                ...
                $class['examine']=0;
                empty($class['id'])?$this->validate($class,'app\validate\Barter'):$this->validate($class,'app\validate\Barter.update');
                $period=getPeriod();
            ...
            foreach ($input['info'] as $infoKey=>$infoVo) {
                try {
                    $this->validate($infoVo,'app\validate\BarterInfo');
                } catch (ValidateException $e) {
            ...
            try {
                ...
                if(empty($class['id'])){
                    //创建数据
                    $createInfo=Barters::create($class);
                    $class['id']=$createInfo['id'];//转存主键
                    Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'新增单据']);
                    pushLog('新增积分兑换单[ '.$class['number'].' ]');//日志
                }else{
                    //更新数据
                    $updateInfo=Barters::update($class);
                    Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'更新单据']);
                    pushLog('更新积分兑换单[ '.$class['number'].' ]');//日志
                }
                
                //INFO数据
                BarterInfo::where([['pid','=',$class['id']]])->delete();
                foreach ($input['info'] as $infoKey=>$infoVo) {
                    $input['info'][$infoKey]['pid']=$class['id'];
                }
                $model = new BarterInfo;
                $model->saveAll($input['info']);
                
                //COST数据
                Cost::where([['type','=','barter'],['class','=',$class['id']]])->delete();
                foreach ($input['cost'] as $costKey=>$costVo) {
                    unset($input['cost'][$costKey]['id']);
                    $input['cost'][$costKey]['type']='barter';
    ...
    public function get(){
        ...
        if(existFull($input,['parm'])){
            $class=Barters::where([['id','=',$input['parm']]])->find();
            $info=BarterInfo::with(['goodsData','warehouseData'])->where([['pid','=',$input['parm']]])->order(['id'=>'asc'])->select();
            $cost=Cost::where([['type','=','barter'],['class','=',$input['parm']]])->order(['id'=>'asc'])->select();
    ...
    public function del(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            $data=Db::name('barter')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            $search=search($data)->where([['examine','=','1']])->find();
            if(empty($search)){
                Db::startTrans();
                try {
                    Db::name('barter')->where([['id','in',$input['parm']]])->delete();
                    Db::name('barter_info')->where([['pid','in',$input['parm']]])->delete();
                    Db::name('cost')->where([['type','=','barter'],['class','in',$input['parm']]])->delete();
                    Db::name('record')->where([['type','=','barter'],['source','in',$input['parm']]])->delete();
                    Db::name('log')->insert(['time'=>time(),'user'=>getUserID(),'info'=>'删除积分兑换单[ '.implode(' | ',array_column($data,'number')).' ]']);
    ...
    public function check(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            $period=getPeriod();
            $classList=Db::name('barter')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            foreach ($input['parm'] as $parmVo) {
                ...
                if(empty($class['check'])){
                    Db::name('barter')->where([['id','=',$class['id']]])->update(['check'=>1]);
                    //1 单据记录
                    Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'核对单据']);
                    //2 记录操作
                    pushLog('核对积分兑换单[ '.$class['number'].' ]');//单据日志
                }else{
                    Db::name('barter')->where([['id','=',$class['id']]])->update(['check'=>0]);
                    //1 单据记录
                    Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'反核对单据']);
                    //2 记录操作
                    pushLog('反核对积分兑换单[ '.$class['number'].' ]');//单据日志
                }
            }
            $result=['state'=>'success'];
        }else{
    ...
    public function examine(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            ...
            $period=getPeriod();
            $classList=Db::name('barter')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            $infoList=Db::name('barter_info')->where([['pid','in',$input['parm']]])->order(['id'=>'asc'])->select()->toArray();
            $customerList=Db::name('customer')->where([['id','in',array_column($classList,'customer')]])->select()->toArray();
            //2 综合处理
            foreach ($input['parm'] as $parmVo) {
                //1 匹配数据
                $class=search($classList)->where([['id','=',$parmVo]])->find();
                ...
                if(empty($class['examine'])){
                    //1 积分验证
                    $customer=search($customerList)->where([['id','=',$class['customer']]],true)->find();
                    if(bccomp($class['actual'],$customer['integral'])==1){
                        return json(['state'=>'error','info'=>'审核单据[ '.$class['number'].' ]失败,原因:客户积分不足!']);
                        exit;
                    }else{
                        $customerList[$customer['rowKey']]['integral']=math()->chain($customer['integral'])->sub($class['actual'])->done();
                    }
                    //2 单据费用
                    $cost=Db::name('cost')->alias('cost')->where([['type','=','barter'],['class','=',$class['id']]])->whereExists(function($query){
                ...
                foreach ($info as $infoKey=>$infoVo) {
                    ...
                    if(empty($goods['type'])){
                        ...
                        if(empty($class['examine'])){
                            ...
                            //1 验证序列号
                            $serialInfoCollect=Db::name('serial_info')->where([['type','=','barter'],['info','in',array_column($info,'id')]])->select()->toArray();
                    ...
                    if(empty($class['examine'])){
                        ...
                        foreach ($info as $infoKey=>$infoVo){
                            ...
                            if(empty($goods['type'])){
                                ...
                                //2 仓储详情
                                $store['roomInfo'][]=['pid'=>null,'type'=>'barter','class'=>$class['id'],'info'=>$infoVo['id'],'time'=>$class['time'],'direction'=>0,'price'=>$infoVo['basic']['price'],'nums'=>$infoVo['basic']['nums']];
                                //3 批次号
                                if(empty($infoVo['batch'])){
                                    $store['batch'][]=[];
                                    $store['batchInfo'][]=[];
                                    ...
                                    $store['batchInfo'][]=['pid'=>null,'type'=>'barter','class'=>$class['id'],'info'=>$infoVo['id'],'direction'=>0,'nums'=>$infoVo['basic']['nums']];
                                }
                                //4 序列号
                                if(empty($infoVo['serial'])){
                                    ...
                                    foreach ($infoVo['serial'] as $serialVo) {
                                        ...
                                        $serialInfo[]=['pid'=>null,'type'=>'barter','class'=>$class['id'],'info'=>$infoVo['id']];
                                    }
                                    $store['serial'][]=$serial;
                                ...
                                $store['serveInfo'][]=['pid'=>null,'type'=>'barter','class'=>$class['id'],'info'=>$infoVo['id'],'time'=>$class['time'],'price'=>$infoVo['price'],'nums'=>$infoVo['nums']];
                            }
                        }
                        //2 仓储
                        if(!empty($store['room'])){
                        ...
                        //10 积分
                        if(!empty($class['actual'])){
                            //1 更新客户积分
                            Db::name('customer')->where([['id','=',$class['customer']]])->dec('integral',$class['actual'])->update();
                            //2 创建积分详情
                            Db::name('integral')->insert([
                                ...
                                'time'=>$class['time'],
                                'direction'=>0,
                                'integral'=>$class['actual']
                            ]);
                        }
                        //11 更新单据
                        Db::name('barter')->where([['id','=',$class['id']]])->update(['examine'=>1]);
                        //12 单据记录
                        Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'审核单据']);
                        //13 收发记录
                        $summary=new Summary;
                        $summary->note('barter',$class['id'],true);
                        //14 记录操作
                        pushLog('审核积分兑换单[ '.$class['number'].' ]');//单据日志
                    }else{
                        //反审核
                        //1 匹配数据
                        $listSql=[['type','=','barter'],['info','in',array_column($info,'id')]];
                        ...
                        if(!empty($class['actual'])){
                            //1 更新客户积分
                            Db::name('customer')->where([['id','=',$class['customer']]])->inc('integral',$class['actual'])->update();
                            //2 删除积分详情
                            Db::name('integral')->where([['type','=','barter'],['class','=',$class['id']]])->delete();
                        }
                        //7 更新单据
                        Db::name('barter')->where([['id','=',$class['id']]])->update(['examine'=>0]);
                        //8 单据记录
                        Db::name('record')->insert(['type'=>'barter','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'反审核单据']);
                        //9 收发记录
                        $summary=new Summary;
                        $summary->note('barter',$class['id'],false);
                        //10 记录操作
                        pushLog('反审核积分兑换单[ '.$class['number'].' ]');//单据日志
                    }
    ...
    public function upload(){
        ...
        if (empty($file)) {
            ...
            try{
                ...
                $fileInfo=Filesystem::disk('upload')->putFile('barter', $file, 'uniqid');
		...
		if(empty($file)){
            ...
            try{
                ...
                if(empty($data[3]['G'])){
                    ...
                    $people=Db::name('people')->where([['name','=',$data[3]['G']]])->find();
                    if(empty($people)){
                        throw new ValidateException('关联人员[ '.$data[3]['G'].' ]未匹配!');
                    }
                }
                $class=[
                ...
                $this->validate($class,'app\validate\Barter');//数据合法性验证
                //初始化INFO
                $info=[];
                ...
                foreach ($data as $dataKey=>$dataVo) {
					...
					//单价匹配
					if(!preg_match("/^\d+(\.\d{0,".$sys['fun']['digit']['money']."})?$/",$record['price'])){
					    throw new ValidateException('模板文件第'.$dataKey.'行单价不正确!');
					}
					//数量匹配
					if(!preg_match("/^\d+(\.\d{0,".$sys['fun']['digit']['nums']."})?$/",$record['nums'])){
					...
					try{
                        $this->validate($record,'app\validate\BarterInfo');//数据合法性验证
                        $storage=math()->chain($record['price'])->mul($record['nums'])->round($sys['fun']['digit']['money'])->done();
                        //折扣额|金额
                        if($record['discount']==0){
                            $record['total']=$storage;
                        }else{
                            $record['dsc']=math()->chain($storage)->div(100)->mul($record['discount'])->round($sys['fun']['digit']['money'])->done();
                            $record['total']=math()->chain($storage)->sub($record['dsc'])->done();
                        }
                        //积分
                        $record['integral']=math()->chain($record['total'])->mul($sys['vend']['barter'])->done();
                        //转存数据
                        $class['total']=math()->chain($class['total'])->add($record['total'])->done();//累加单据金额
                        $class['integral']=math()->chain($class['integral'])->add($record['integral'])->done();//累加单据积分
                        $info[]=$record;
					} catch (ValidateException $e) {
                ...
                //CLASS数据验证
                if(bccomp($class['integral'],$class['actual'])==-1){
                    throw new ValidateException('实收积分不可大于单据积分[ '.floatval($class['integral']).' ]!');
                }else{
                    Db::startTrans();
                    try {
                        //新增CLASS
                        $classData=Barters::create($class);
                        //新增INFO
                        foreach ($info as $infoKey=>$infoVo) {
                        ...
                        $model = new BarterInfo;
                        $model->saveAll($info);
                        Db::name('record')->insert(['type'=>'barter','source'=>$classData['id'],'time'=>time(),'user'=>getUserID(),'info'=>'导入单据']);
                        pushLog('导入积分兑换单[ '.$classData['number'].' ]');//日志
	...
	public function exports(){
		...
		if(existFull($input,['scene','parm']) && is_array($input['parm'])){
		    pushLog('导出积分兑换单列表');//日志
            $source=Barters::with(['frameData','customerData','peopleData','userData','recordData'])->where([['id','in',$input['parm']]])->append(['extension'])->order(['id'=>'desc'])->select()->toArray();//查询CLASS数据
            if($input['scene']=='simple'){
                //简易报表
                //开始构造导出数据
                $excel=[];//初始化导出数据
                //标题数据
                $excel[]=['type'=>'title','info'=>'积分兑换单列表'];
                //表格数据
                $field=[
                	'frameData|name'=>'所属组织',
                ...
                $excel[]=['type'=>'node','info'=>[
                    ...
                    '总实收积分:'.mathArraySum(array_column($source,'actual')),
                    '总单据费用:'.mathArraySum(array_column($source,'cost'))
                ]];
                //导出execl
                buildExcel('积分兑换单列表',$excel);
            }else{
                //详细报表
                $files=[];//初始化文件列表
                foreach ($source as $sourceVo) {
                    ...
                    //构造表内数据
                    $info=BarterInfo::with(['goodsData','warehouseData'])->where([['pid','=',$sourceVo['id']]])->order(['id'=>'asc'])->append(['extension'])->select()->toArray();
                    //批次号匹配
