...
use app\model\{Vre as Vres,VreInfo,Cost,Goods,Account};
use think\facade\{Db,Filesystem};
use think\exception\ValidateException;
class Vre extends Acl{
    //列表
    public function record(){
        ...
        if(existFull($input,['page','limit'])){
            ...
            if(existFull($input,['goods'])){
                ...
                $sql[]=['id','in',array_column(Db::name('vre_info')->where([['goods','in',$goods]])->select()->toArray(),'pid')];
            }
            $sql=frameScope($sql);//组织数据
            $sql=sqlAuth('vre',$sql);//数据鉴权
            $count = Vres::where($sql)->count();//获取总条数
            $info = Vres::with(['frameData','customerData','peopleData','userData','costData','invoiceData','recordData'])->where($sql)->append(['extension'])->page($input['page'],$input['limit'])->order(['id'=>'desc'])->select()->toArray();//查询分页数据
            //关联单据
            if(!empty($info)){
                $vend=Db::name('vend')->where([['id','in',array_column($info,'source')]])->select()->toArray();
                foreach ($info as $infoKey=>$infoVo) {
                    //零售单
                    $relation=array_map(function($item){
                        return ['type'=>'零售单','time'=>date('Y-m-d',$item['time']),'number'=>$item['number'],'sort'=>$item['time'],'sort'=>$item['time'],'types'=>'vend','id'=>$item['id']];
                    },search($vend)->where([['id','=',$infoVo['source']]])->select());
                    //数据排序
    ...
    public function save(){
        ...
        if(existFull($input,['class','info']) && isset($input['class']['id']) && isset($input['cost'])){
            ...
            try {
                ...
                $class['examine']=0;
                empty($class['id'])?$this->validate($class,'app\validate\Vre'):$this->validate($class,'app\validate\Vre.update');
                $period=getPeriod();
            ...
            foreach ($input['info'] as $infoKey=>$infoVo) {
                try {
                    $this->validate($infoVo,'app\validate\VreInfo');
                } catch (ValidateException $e) {
            ...
            try {
                ...
                if(empty($class['id'])){
                    //创建数据
                    $createInfo=Vres::create($class);
                    $class['id']=$createInfo['id'];//转存主键
                    Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'新增单据']);
                    pushLog('新增零售退货单[ '.$class['number'].' ]');//日志
                }else{
                    //更新数据
                    $updateInfo=Vres::update($class);
                    Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'更新单据']);
                    pushLog('更新零售退货单[ '.$class['number'].' ]');//日志
                }
                
                //INFO数据
                VreInfo::where([['pid','=',$class['id']]])->delete();
                foreach ($input['info'] as $infoKey=>$infoVo) {
                ...
                //COST数据
                Cost::where([['type','=','vre'],['class','=',$class['id']]])->delete();
                foreach ($input['cost'] as $costKey=>$costVo) {
                    unset($input['cost'][$costKey]['id']);
                    $input['cost'][$costKey]['type']='vre';
    ...
    public function get(){
        ...
        if(existFull($input,['parm'])){
            $class=Vres::where([['id','=',$input['parm']]])->find();
            $info=VreInfo::with(['goodsData','warehouseData'])->where([['pid','=',$input['parm']]])->order(['id'=>'asc'])->select();
            $cost=Cost::where([['type','=','vre'],['class','=',$input['parm']]])->order(['id'=>'asc'])->select();
    ...
    public function del(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            $data=Db::name('vre')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            $search=search($data)->where([['examine','=','1']])->find();
            if(empty($search)){
                Db::startTrans();
                try {
                    Db::name('vre')->where([['id','in',$input['parm']]])->delete();
                    Db::name('vre_info')->where([['pid','in',$input['parm']]])->delete();
                    Db::name('cost')->where([['type','=','vre'],['class','in',$input['parm']]])->delete();
                    Db::name('record')->where([['type','=','vre'],['source','in',$input['parm']]])->delete();
                    Db::name('log')->insert(['time'=>time(),'user'=>getUserID(),'info'=>'删除零售退货单[ '.implode(' | ',array_column($data,'number')).' ]']);
    ...
    public function check(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            $period=getPeriod();
            $classList=Db::name('vre')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            foreach ($input['parm'] as $parmVo) {
                ...
                if(empty($class['check'])){
                    Db::name('vre')->where([['id','=',$class['id']]])->update(['check'=>1]);
                    //1 单据记录
                    Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'核对单据']);
                    //2 记录操作
                    pushLog('核对零售退货单[ '.$class['number'].' ]');//单据日志
                }else{
                    Db::name('vre')->where([['id','=',$class['id']]])->update(['check'=>0]);
                    //1 单据记录
                    Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'反核对单据']);
                    //2 记录操作
                    pushLog('反核对零售退货单[ '.$class['number'].' ]');//单据日志
                }
            }
            $result=['state'=>'success'];
        }else{
    ...
    public function examine(){
        ...
        if(existFull($input,['parm']) && is_array($input['parm'])){
            ...
            $period=getPeriod();
            $classList=Db::name('vre')->where([['id','in',$input['parm']]])->order(['id'=>'desc'])->select()->toArray();
            $infoList=Db::name('vre_info')->where([['pid','in',$input['parm']]])->order(['id'=>'asc'])->select()->toArray();
            //2 综合处理
            foreach ($input['parm'] as $parmVo) {
                //1 匹配数据
                $class=search($classList)->where([['id','=',$parmVo]])->find();
                ...
                if(empty($class['examine'])){
                    //1 关联单据
                    empty($class['source'])||$vendInfoList=Db::name('vend_info')->where([['id','in',array_column($info,'source')]])->select()->toArray();
                    //2 构造数据
                    $batchGather=[];
                    $serialGather=[];
                    $roomWhereOrSql=[];
                ...
                if(!empty($class['examine'])){
                    //1 单据费用
                    $cost=Db::name('cost')->alias('cost')->where([['type','=','vre'],['class','=',$class['id']]])->whereExists(function($query){
                    ...
                    //2 单据发票
                    $invoice=Db::name('invoice')->where([['type','=','vre'],['class','=',$class['id']]])->find();
                    if(!empty($invoice)){
                    ...
                    if(empty($goods['type'])){
                        ...
                        if(empty($class['examine'])){
                            ...
                            if(!empty($infoVo['source'])){
                                $vendInfo=search($vendInfoList)->where([['id','=',$infoVo['source']]])->find();
                                if(!empty($vendInfo)){
                                    if($vendInfo['unit']!=$infoVo['unit']){
                                        //单位匹配
                                        return json(['state'=>'error','info'=>'审核单据[ '.$class['number'].' ]失败,原因:第'.($infoKey+1).'行单位与零售单不匹配!']);
                                        exit;
                                    }elseif(bccomp(math()->chain($vendInfo['retreat'])->add($infoVo['nums'])->done(),$vendInfo['nums'])==1){
                                        //数量匹配
                                        return json(['state'=>'error','info'=>'审核单据[ '.$class['number'].' ]失败,原因:第'.($infoKey+1).'行超出零售单可退货数量!']);
                                        exit;
                                    }
                                }
                            ...
                            //1 验证序列号
                            $serialInfoCollect=Db::name('serial_info')->where([['type','=','vre'],['info','in',array_column($info,'id')]])->select()->toArray();
                ...
                try {
                    ...
                    if(empty($class['examine'])){
                        //审核
                        //1 构造数据
                        $store=['vendInfo'=>[],'room'=>[],'roomInfo'=>[],'batch'=>[],'batchInfo'=>[],'serial'=>[],'serialInfo'=>[],'serve'=>[],'serveInfo'=>[]];
                        foreach ($info as $infoKey=>$infoVo){
                            //1 关联单据
                            empty($infoVo['source'])||$store['vendInfo'][]=['id'=>$infoVo['source'],'retreat'=>$infoVo['nums']];
                            //2 判断商品类型
                            $goods=search($goodsList)->where([['id','=',$infoVo['goods']]])->find();
                            if(empty($goods['type'])){
                                //常规商品
                                ...
                                //2 仓储详情
                                $store['roomInfo'][]=['pid'=>null,'type'=>'vre','class'=>$class['id'],'info'=>$infoVo['id'],'time'=>$class['time'],'direction'=>1,'price'=>$infoVo['basic']['price'],'nums'=>$infoVo['basic']['nums']];
                                //3 批次号
                                if(empty($infoVo['batch'])){
                                    ...
                                    $store['batchInfo'][]=['pid'=>null,'type'=>'vre','class'=>$class['id'],'info'=>$infoVo['id'],'direction'=>1,'nums'=>$infoVo['basic']['nums']];
                                }
                                //4 序列号
                                if(empty($infoVo['serial'])){
                                    ...
                                    foreach ($infoVo['serial'] as $serialVo) {
                                        ...
                                        $serialInfo[]=['pid'=>null,'type'=>'vre','class'=>$class['id'],'info'=>$infoVo['id']];
                                    }
                                    $store['serial'][]=$serial;
                                ...
                                $store['serveInfo'][]=['pid'=>null,'type'=>'vre','class'=>$class['id'],'info'=>$infoVo['id'],'time'=>$class['time'],'price'=>$infoVo['price'],'nums'=>$infoVo['nums']];
                            }
                        }
                        //2 关联单据
                        if(!empty($store['vendInfo'])){
                            //更新详情
                            Db::name('vend_info')->duplicate(['retreat'=>Db::raw('retreat + VALUES(`retreat`)')])->insertAll($store['vendInfo']);
                        }
                        //3 仓储
                        if(!empty($store['room'])){
                            //1 构造数据
                        ...
                        //13 更新单据
                        Db::name('vre')->where([['id','=',$class['id']]])->update(['examine'=>1]);
                        //14 单据记录
                        Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'审核单据']);
                        //15 收发记录
                        $summary=new Summary;
                        $summary->note('vre',$class['id'],true);
                        //16 记录操作
                        pushLog('审核零售退货单[ '.$class['number'].' ]');//单据日志
                    }else{
                        //反审核
                        //1 匹配数据
                        $listSql=[['type','=','vre'],['info','in',array_column($info,'id')]];
                        ...
                        if(!empty($class['source'])){
                            ...
                            $vendInfoDuplicate=[];
                            foreach ($info as $infoVo) {
                                empty($infoVo['source'])||$vendInfoDuplicate[]=['id'=>$infoVo['source'],'retreat'=>$infoVo['nums']];
                            }
                            empty($vendInfoDuplicate)||Db::name('vend_info')->duplicate(['retreat'=>Db::raw('retreat - VALUES(`retreat`)')])->insertAll($vendInfoDuplicate);
                        }
                        //3 仓储
                        $roomDuplicate=[];
                        ...
                        if($class['actual']!=0){
                            ...
                            //2 删除资金详情
                            Db::name('account_info')->where([['type','=','vre'],['class','=',$class['id']]])->delete();
                        }
                        //8 积分
                        if(!empty($class['integral'])){
                            //1 更新客户积分
                            ...
                            //2 删除积分详情
                            Db::name('integral')->where([['type','=','vre'],['class','=',$class['id']]])->delete();
                        }
                        //9 更新单据
                        Db::name('vre')->where([['id','=',$class['id']]])->update(['examine'=>0]);
                        //10 单据记录
                        Db::name('record')->insert(['type'=>'vre','source'=>$class['id'],'time'=>time(),'user'=>getUserID(),'info'=>'反审核单据']);
                        //11 收发记录
                        $summary=new Summary;
                        $summary->note('vre',$class['id'],false);
                        //12 记录操作
                        pushLog('反审核零售退货单[ '.$class['number'].' ]');//单据日志
                    }
    ...
    public function upload(){
        ...
        if (empty($file)) {
            ...
            try{
                ...
                $fileInfo=Filesystem::disk('upload')->putFile('vre', $file, 'uniqid');
    ...
    public function import(){
		...
		if(empty($file)){
            ...
            try{
                ...
                $this->validate($class,'app\validate\Vre');//数据合法性验证
                //初始化INFO
                $info=[];
                ...
                foreach ($data as $dataKey=>$dataVo) {
					...
					try{
                        $this->validate($record,'app\validate\VreInfo');//数据合法性验证
                ...
                if(bccomp($class['total'],$class['actual'])==-1){
                    ...
                    try {
                        ...
                        $model = new VreInfo;
                        $model->saveAll($info);
                        Db::name('record')->insert(['type'=>'vre','source'=>$classData['id'],'time'=>time(),'user'=>getUserID(),'info'=>'导入单据']);
                        pushLog('导入零售退货单[ '.$classData['number'].' ]');//日志
	...
	public function exports(){
		...
		if(existFull($input,['scene','parm']) && is_array($input['parm'])){
		    pushLog('导出零售退货单列表');//日志
            $source=Vres::with(['frameData','customerData','accountData','peopleData','userData'])->where([['id','in',$input['parm']]])->append(['extension'])->order(['id'=>'desc'])->select()->toArray();//查询CLASS数据
            if($input['scene']=='simple'){
                //简易报表
                //开始构造导出数据
                $excel=[];//初始化导出数据
                //标题数据
                $excel[]=['type'=>'title','info'=>'零售退货单列表'];
                //表格数据
                $field=[
                	'frameData|name'=>'所属组织',
                ...
                foreach ($source as $sourceVo) {
                    ...
                    //构造表内数据
                    $info=VreInfo::with(['goodsData','warehouseData'])->where([['pid','=',$sourceVo['id']]])->order(['id'=>'asc'])->append(['extension'])->select()->toArray();
                    //批次号匹配
