...
class Barter extends Model{
    ...
    public function costData(){
        return $this->hasMany(Cost::class,'class','id')->with(['ietData'])->where([['type','=','barter']])->append(['extension'])->order('id desc');
    }
    
    //记录关联
    public function recordData(){
        return $this->hasMany(Record::class,'source','id')->with(['userData'])->where([['type','=','barter']])->append(['extension'])->order('id desc');
    }
